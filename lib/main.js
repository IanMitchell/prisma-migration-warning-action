...
async function getSchemaRemovalCount(mainBranch, path) {
    const { stdout } = await $ `git diff $(git log -n 1 origin/${mainBranch} --pretty=format:"%H") $(git log -n 1 --pretty=format:"%H") --numstat ${path}/prisma.schema | awk '{ print $2}'`;
    return parseInt(stdout.trim(), 10);
}
async function getModifiedFileCount(mainBranch, path) {
    const { stdout } = await $ `git diff $(git log -n 1 origin/${mainBranch} --pretty=format:"%H") $(git log -n 1 --pretty=format:"%H") --numstat -- . :^${path} | wc -l`;
    return parseInt(stdout.trim(), 10);
}
async function run() {
    try {
